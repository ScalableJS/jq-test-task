# Web Developer Test
# Demo page
https://wxx4cz.csb.app/

![Mobile](https://github.com/ScalableJS/jq-test-task/blob/static-page/screenshots/mobile.png)

![Desktop](https://github.com/ScalableJS/jq-test-task/blob/static-page/screenshots/desktop.png)

### Task description
Reference the file "concept.jpg".  This is what index.html should look like when completed and viewed in browser.

All assets are included in this main directory. Blank CSS file and .js files are also included.  Simply work the design by manipulating the HTML as needed to include any Classes/ID's you need to finish the task.

Be mindful of responsive design, hover states on anything that is linked, and add some jQuery to manipulate the HTML and add a different look to the header as the user scrolls.  The header should be a fixed position on desktop view.
